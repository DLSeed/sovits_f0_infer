...
async def infer(speaker: str, tran: int, audio: bytes = File(..., max_length=2097152)):
    target_sample = hps_ms.data.sampling_rate
    with open(f"{http_temp_path}/http_in.wav", "wb") as f:
        f.write(audio)
    out_audio, out_sr = infer_tool.infer(f"{http_temp_path}/http_in.wav", speaker, tran, net_g_ms, hubert_soft,
                                         feature_input)
